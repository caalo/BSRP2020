...
Let's actually some data to explore this issue and see what we can do about it. Take 5 WT and 5 MT cell lines and make comparison for 1,000 genes. 
...
for(i in 1:n_genes) {
  WT_exp <- rnorm(n_cell_lines, mean = 50, sd = 1)
  MT_exp <- rnorm(n_cell_lines, mean = 50, sd = 1)
...
### Scenario 2: 20% of genes are diffrentially expressed. 
...
for(i in (n_genes*(1 - pct_DE) + 1):n_genes) {
  WT_exp <- rnorm(n_cell_lines, mean = 50, sd = 1)
  MT_exp <- rnorm(n_cell_lines, mean = 53, sd = 1)
  df <- data.frame(gene = paste0("gene ", i), 
                   cell_line = c(rep("WT", n_cell_lines), rep("MT", n_cell_lines)),
                   exp = c(WT_exp, MT_exp), 
...
```{r, echo=F}
cat("True Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value >= .05)), "\n") 
cat("False Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value < .05)), "\n") 
cat("True Negatives: ", length(which(gene_exp_test$null_hypothesis == F & gene_exp_test$p_value < .05)), "\n")
...
cat("True Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value >= .05/1000)), "\n") 
cat("False Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value < .05/1000)), "\n") 
cat("True Negatives: ", length(which(gene_exp_test$null_hypothesis == F & gene_exp_test$p_value < .05/1000)), "\n")
cat("False Negatives: ", length(which(gene_exp_test$null_hypothesis == F & gene_exp_test$p_value >= .05/1000)), "\n")
```

We got rid of *all* of our False Positives, but we have become so conservative of making a single error that we lost almost all of our ability to detect True Negatives. We have cell lines that have differentially expression (200 of them), and now we can only call a few of them as differentially expressed because we were so careful of having a single false positive. 

We talk about False Positives, and now we're paying more attention to False Negatives. Here's a commonly used term in statistics: the **power** of a test is True Negatives / (False Negatives + True Negatives). What is the power of the test here? Is it low or high?
...
cat("True Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value_corrected >= .05)), "\n") 
cat("False Positives: ", length(which(gene_exp_test$null_hypothesis == T & gene_exp_test$p_value_corrected < .05)), "\n") 
cat("True Negatives: ", length(which(gene_exp_test$null_hypothesis == F & gene_exp_test$p_value_corrected < .05)), "\n")
