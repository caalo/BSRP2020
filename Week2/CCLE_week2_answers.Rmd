...
IMPORTANT NOTE: To run the following scripts on the full data (on your local machine), uncomment lines 48 and 54 below.
...
# random_cols <- sample(colnames(CCLE_expression), size = 98, replace = F)
# CCLE_expression_subset <- CCLE_expression[,c("X1", random_cols, "KRAS (3845)","EGFR (1956)")]
# write_csv(CCLE_expression_subset, "CCLE_expression-subset.csv")

CCLE_mutations <- read_csv("CCLE_mutations-subset.csv")
# CCLE_mutations <- read_csv("CCLE_mutations.csv")
# CCLE_mutations %>%
#    filter(Hugo_Symbol %in% c("KRAS", "EGFR")) %>% 
#    write_csv("CCLE_mutations-subset.csv")
```
...
ggplot(data = subsetted_data, aes(x = KRAS_mutated, y = KRAS_expression)) +
  geom_boxplot() +
  labs(x = "KRAS mutated?", y = "KRAS expression", title = "KRAS expression in lung cancer cell lines")
...
t.test(KRAS_expression ~ KRAS_mutated, data = subsetted_data)

# Yes, the t.test shows that there is a significant difference 
# between KRAS expression in the KRAS mutated and wildtype cell lines
```
...
ggplot(data = EGFR_data, aes(x = EGFR_mutated, y = EGFR_expression)) +
  geom_boxplot() # doesn't look meaninfully different...

# Test if these two are statistically significantly different
